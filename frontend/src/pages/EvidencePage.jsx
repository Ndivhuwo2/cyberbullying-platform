  ...
  return (
    <div style={{ maxWidth: '700px', margin: '60px auto', padding: '0 20px' }}>
      ...
      <div style={{ border: '1px solid #444', borderRadius: '8px', padding: '16px', marginBottom: '32px' }}>
        ...
        <input type="file" onChange={e => setFile(e.target.files[0])} style={{ display: 'block', marginBottom: '12px' }} />
        {error && <p style={{ color: 'red' }}>{error}</p>}
        <button onClick={handleUpload} disabled={uploading}>{uploading ? 'Uploading...' : 'Upload File'}</button>
      </div>
      ...
      {evidence.length === 0 ? (
        ...
        evidence.map(e => (
          <div key={e.id} style={{ border: '1px solid #444', borderRadius: '8px', padding: '12px', marginBottom: '12px' }}>
            <p style={{ margin: '0 0 4px', fontWeight: 'bold' }}>{e.file_name}</p>
            <p style={{ margin: '0 0 4px', fontSize: '13px', color: '#aaa' }}>SHA256: {e.sha256_hash}</p>
            <a href={e.file_url} target="_blank" rel="noreferrer" style={{ fontSize: '13px' }}>View File</a>
          </div>
        ))
      )}
    </div>
